...
from schemas.movies import MovieFilterSchema, MovieListItemSchema
...
async def test_get_favorite_movies_without_params(
    ...
    response = await client.get(
        "/favorites/", headers={"Authorization": f"Bearer {access_token}"}
    )
    ...
    movies = await db_session.scalars(
        (
            select(MovieModel)
            .join(
                MoviesFavoritesModel, MovieModel.id == MoviesFavoritesModel.c.movie_id
            )
...
async def test_get_favorite_movies_with_genres_params(
    ...
    movies = await db_session.scalars(
        query.join(MovieModel.genres).where(GenreModel.id.in_(genres))
    )
    movies = movies.all()
...
async def test_user_get_movies_without_favorite_list(
    ...
    response_data = response.json()
    assert (
        response_data["detail"] == "You don't have favorite movies."
    ), "Response data does not match"
...
async def test_user_get_movies_favorite_without_movies(
    ...
    response_data = response.json()
    assert (
        response_data["detail"] == "Your favorite movies list is empty."
    ), "Response data does not match"
...
async def test_user_get_empty_movies_favorite_list(
    ...
    filters = {"page": 1, "per_page": 10, "search": "XMVKFME"}
    db_user = await db_session.scalar(select(UserModel).where(UserModel.id == 1))
    access_token = jwt_manager.create_access_token(
        data={"email": db_user.email, "user_id": db_user.id}
    )
...
async def test_add_movie_to_new_favorite_list(
    ...
    favorite = await db_session.scalar(
        select(FavoriteModel).where(FavoriteModel.user_id == create_test_user.id)
    )
    favorite_movie = await db_session.scalar(
        select(func.count())
        .select_from(MoviesFavoritesModel)
        .where(
...
async def test_add_movie_to_favorite_twice(
    ...
    assert (
        second_response_data["detail"] == "Movie already in favorites."
    ), "Response data does not match"
...
async def test_unknown_user_add_movie_to_favorite(
    ...
    response = await client.post("/favorites/1/", json=payload)
...
async def test_add_movie_to_favorite_sqlalchemy_error(
    ...
    with patch("routes.favorites.AsyncSession.commit", side_effect=SQLAlchemyError):
        ...
        assert (
            ...
            == "An error occurred while adding the movie to favorites."
        )
...
async def test_remove_movie_not_favorite_movie_from_favorite_list(
    ...
    assert (
        delete_response_data["detail"] == "Movie is not in favorites."
    ), "Response data does not match"
