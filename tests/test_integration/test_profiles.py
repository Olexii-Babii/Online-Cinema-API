...
async def test_create_user_profile_with_fake_s3(
    ...
    profile = await db_session.scalar(select(UserProfileModel).where(UserProfileModel.user_id == user.id))
...
async def test_create_user_profile_expired_token(client, jwt_manager):
    ...
    assert response.status_code == 401, f"Expected status code does not match. Should be 401"
...
async def test_admin_creates_user_profile(
    ...
    admin_payload = {
        "email": "testadmin@emaple.com",
        "password": "AdminPassword12345@"
    }

    user_group = await db_session.scalar(select(UserGroupModel).where(UserGroupModel.name == UserGroupEnum.USER))

    admin_group = await db_session.scalar(select(UserGroupModel).where(UserGroupModel.name == UserGroupEnum.ADMIN))
    ...
    users = await db_session.scalars(select(UserModel).where(UserModel.email.in_(
        [user_payload["email"], admin_payload["email"]]
    )))
    ...
    profile = await db_session.scalar(select(UserProfileModel).where(UserProfileModel.user_id == regular_user.id))
...
async def test_user_cannot_create_another_user_profile(
    ...
    user1_payload = {
        "email": "testuser1@emaple.com",
        "password": "User1Password12345@"
    }

    user2_payload = {
        "email": "testuser2@emaple.com",
        "password": "User2Password12345@"
    }
    ...
    users = await db_session.scalars(select(UserModel).where(UserModel.email.in_(
        [user1_payload["email"], user2_payload["email"]]
    )))
    ...
    response = await client.post(profile_url, headers=headers, files=files)
    assert response.status_code == 403, f"Expected status code does not match. Should be 403"
    assert response.json()["detail"] == "You don't have permission to edit this profile.", \
        f"Unexpected error message: {response.json()['detail']}"

    profile = await db_session.scalar(select(UserProfileModel).where(UserProfileModel.user_id == user_2.id))
...
async def test_cannot_create_profile_twice(
    ...
    assert response1.status_code == 201, f"Expected status code does not match. Should be 201"

    response2 = await client.post(profile_url, headers=headers, files=files)
    assert response2.status_code == 400, f"Expected status code does not match. Should be 400"
    assert response2.json()["detail"] == "User already has a profile.", (
        f"Unexpected error message: {response2.json()['detail']}"
    )

    count = await db_session.scalar(select(func.count(UserProfileModel.id)).where(UserProfileModel.user_id == user.id))
...
async def test_profile_creation_fails_on_s3_upload_error(
    ...
    with patch.object(s3_storage_fake, "upload_image", side_effect=ConnectionError("Simulated S3 failure")):
        response = await client.post(profile_url, headers=headers, files=files)

    assert response.status_code == 500, f"Expected status code does not match. Should be 500"
...
async def test_profile_creation_invalid_name(client, jwt_manager, first_name, last_name, expected_error):
    ...
    assert response.status_code == 422, f"Expected status code does not match. Should be 422"
...
async def test_profile_creation_invalid_gender(client, jwt_manager):
    ...
    assert response.status_code == 422, f"Expected status code does not match. Should be 422"
    assert "Gender must be either 'man' or 'woman'" in str(response.json()), f"Unexpected error message: {response.json()}"
