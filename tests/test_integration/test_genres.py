...
async def test_get_all_genres(
    ...
    genres = await db_session.execute(select(GenreModel.name, func.count(MovieModel.id).label("movie_count"))
                              .outerjoin(GenreModel.movies)
                              .group_by(GenreModel.id))
    genres = genres.mappings().all()

    response = await client.get("/genres/", headers={"Authorization": f"Bearer {access_token}"})
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_unknown_user_get_all_genres(
...
async def test_moder_create_genre(
    ...
    response = await client.post("/genres/", headers={"Authorization": f"Bearer {access_token}"}, json=payload)
...
async def test_create_genre_sqlalchemy_error(
    ...
    with (patch("routes.genres.AsyncSession.commit", side_effect=SQLAlchemyError)):
        ...
        response = await client.post("/genres/", json=payload, headers={"Authorization": f"Bearer {access_token}"})

        assert response.status_code == 500, "Expected status code does not match. Should be 500"
        response_data = response.json()
        assert response_data == {"detail": "An error occurred while creating the genre."}, \
...
async def test_moder_update_genre(
    ...
    genre = await db_session.scalar(select(GenreModel).where(GenreModel.name == payload["name"]))
    ...
    new_response = await client.patch(
        f"/genres/{genre.id}/", json=new_payload, headers={"Authorization": f"Bearer {access_token}"}
    )
...
async def test_moder_update_not_existing_genre(
    ...
    new_response = await client.patch(
        f"/genres/1000/", json=payload, headers={"Authorization": f"Bearer {access_token}"}
    )

    assert new_response.status_code == 404, "Expected status code does not match. Should be 404"
    response_data = new_response.json()
    assert response_data["detail"] == "Genre with the given ID was not found.", "Response data does not match"
...
async def test_user_update_genre(
    ...
    response = await client.post("/genres/", headers={"Authorization": f"Bearer {moder_access_token}"}, json=payload)
    ...
    response = await client.patch(
        f"/genres/{genre.id}/", headers={"Authorization": f"Bearer {user_access_token}"}, json=new_payload
    )
...
async def test_moder_update_genre_with_same_existing_name(
    ...
    genres = await db_session.scalars(select(GenreModel).where(
        or_(
            GenreModel.id == 1,
            GenreModel.id == 2
        )))
    ...
    new_response = await client.patch(
        f"/genres/{genres[0].id}/", json=payload, headers={"Authorization": f"Bearer {access_token}"}
    )

    assert new_response.status_code == 409, "Expected status code does not match. Should be 409"
    response_data = new_response.json()
    assert response_data["detail"] == f"Genre with this name ({payload["name"]}) already exists.", \
        "Response data does not match"
...
async def test_update_genre_sqlalchemy_error(
    ...
    with (patch("routes.genres.AsyncSession.commit", side_effect=SQLAlchemyError)):
        ...
        response = await client.patch("/genres/1/", json=payload, headers={"Authorization": f"Bearer {access_token}"})

        assert response.status_code == 500, "Expected status code does not match. Should be 500"
        response_data = response.json()
        assert response_data == {"detail": "An error occurred while updating the genre."}, \
        "Expected message does not match"
...
async def test_moder_delete_genre(
    ...
    delete_response = await client.delete(f"/genres/{genre.id}/", headers={"Authorization": f"Bearer {access_token}"})

    assert delete_response.status_code == 204, "Expected status code does not match. Should be 204"

    deleted_genre = await db_session.scalar(select(GenreModel).where(GenreModel.name == payload["name"]))
...
async def test_user_delete_genre(
    ...
    response = await client.delete("/genres/1/", headers={"Authorization": f"Bearer {access_token}"})
...
async def test_moder_delete_genre_twice(
    ...
    first_response = await client.delete(f"/genres/{genre.id}/", headers={"Authorization": f"Bearer {access_token}"})
    ...
    second_response = await client.delete(f"/genres/{genre.id}/", headers={"Authorization": f"Bearer {access_token}"})

    assert second_response.status_code == 404, "Expected status code does not match. Should be 404"

    second_response_data = second_response.json()
    assert second_response_data["detail"] == "Genre with the given ID was not found.", "Response data does not match"
...
async def test_moder_delete_not_existing_genre(
    ...
    response = await client.delete("/genres/100000/", headers={"Authorization": f"Bearer {access_token}"})
...
async def test_moder_delete_genre_with_movies(
    ...
    second_response_data = response.json()
    assert second_response_data["detail"] == "There are already films with this genre.", "Response data does not match"
...
async def test_delete_genre_sqlalchemy_error(
    ...
    await client.post("/genres/", headers={"Authorization": f"Bearer {access_token}"}, json=payload)

    genre = await db_session.scalar(select(GenreModel).where(GenreModel.name == payload["name"]))

    with (patch("routes.genres.AsyncSession.commit", side_effect=SQLAlchemyError)):

        response = await client.delete(f"/genres/{genre.id}/", headers={"Authorization": f"Bearer {access_token}"})

        assert response.status_code == 500, "Expected status code does not match. Should be 500"
        response_data = response.json()
        assert response_data == {"detail": "An error occurred while deleting the genre."}, \
        "Expected message does not match"
