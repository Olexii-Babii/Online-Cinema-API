...
async def test_registration(
    ...
    mailhog_url = (
        f"http://{settings.MAIL_SERVER}:{settings.MAIL_API_PORT}/api/v2/messages"
    )
    ...
    assert validate_url(
        activation_url
    ), f"The extracted URL '{activation_url}' is not valid!"
...
async def test_account_activation(e2e_client, settings, e2e_db_session):
    ...
    activation_token_record = await e2e_db_session.scalar(
    ...
    ), f"Activation token for email {payload['email']} not found!"
    token_value = activation_token_record.token

    activation_url = "/accounts/activate/"
    response = await e2e_client.post(
        activation_url, json={"email": payload["email"], "token": token_value}
    )
    assert (
    ...
    activated_user = await e2e_db_session.scalar(
    ...
    assert validate_url(
        activation_complete_url
    ), f"The extracted URL '{activation_complete_url}' is not valid!"
...
async def test_user_login(e2e_client, e2e_db_session):
    ...
    stored_token = await e2e_db_session.scalar(
    ...
    assert (
        stored_token.user.email == payload["email"]
...
async def test_request_password_reset(e2e_client, e2e_db_session, settings):
    ...
    reset_token = await e2e_db_session.scalar(
        select(PasswordResetTokenModel).join(UserModel).where(UserModel.email == email)
    )
    ...
    assert (
        email in email_data["Content"]["Headers"]["To"][0]
    ), "Recipient email does not match!"
...
async def test_reset_password(e2e_client, e2e_db_session, settings):
    ...
    assert (
        reset_token_record
    ), f"Password reset token for email {payload["email"]} was not found!"
    ...
    deleted_token = await e2e_db_session.scalar(
    ...
    updated_user = await e2e_db_session.scalar(
        select(UserModel).where(UserModel.email == payload["email"])
    )
    ...
    email_data = messages[0]
    assert (
        payload["email"] in email_data["Content"]["Headers"]["To"][0]
    ), "Recipient email does not match!"
    email_subject = email_data["Content"]["Headers"].get("Subject", [None])[0]
    assert (
    ...
    assert validate_url(
        reset_complete_url
    ), f"The extracted URL '{reset_complete_url}' is not valid!"
