...
async def create_test_moder(db_session: AsyncSession, seed_database):
    payload = {"email": "testmoder@example.com", "password": "Password12345@"}

    moder_group = await db_session.scalar(
        select(UserGroupModel).where(UserGroupModel.name == UserGroupEnum.MODERATOR)
    )

    moder = UserModel(
        email=payload["email"], password=payload["password"], group_id=moder_group.id
    )
...
def mock_celery_tasks():
    with patch(
        "routes.accounts.delay_delete_activation_token.apply_async"
    ) as mocked_task:
        yield mocked_task
